...
const FooterSection = () => {
  ...
  const [columns, setColumns] = useState([
    {
      title: "Produkt",
      links: ["Funksjoner", "Priser", "API", "Integrasjoner"]
    },
    {
      title: "Ressurser",
      links: ["Blog", "Guider", "Webinarer", "Hjelp & Support"]
    },
    {
      title: "Selskap",
      links: ["Om oss", "Karriere", "Kontakt", "Personvern"]
    }
  ]);
  ...
  const handleLinkTextChange = (columnIndex: number, linkIndex: number, newText: string) => {
    const newColumns = [...columns];
    newColumns[columnIndex].links[linkIndex] = newText;
    setColumns(newColumns);
  };

  const [copyrightText, setCopyrightText] = useState("NewsletterHub. Alle rettigheter reservert.");
        ...
        <div className="grid md:grid-cols-4 gap-8 mb-12">
          ...
          {columns.map((column, columnIndex) => (
            <div key={columnIndex}>
              <EditableText 
                text={column.title}
                onSave={(newTitle) => handleColumnTitleChange(columnIndex, newTitle)}
                ...
                {column.links.map((link, linkIndex) => (
                  <li key={linkIndex}>
                    <a href="#" className="text-gray-400 hover:text-blue-400">
                      <EditableText 
                        text={link}
                        onSave={(newText) => handleLinkTextChange(columnIndex, linkIndex, newText)}
