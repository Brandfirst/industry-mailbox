  ...
  try {
    ...
    const logsWithEmailInfo = data.map(log => {
      ...
      const typedLog: SyncLogEntry = {
        ...
        details: log.details ? JSON.parse(JSON.stringify(log.details)) : null, // Safely clone the details object
      ...
      if (typedLog.account?.email && typedLog.details) {
        typedLog.details = {
          ...(typedLog.details as object), // Cast to object before spreading
