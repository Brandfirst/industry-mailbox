...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    if (user && !authLoading) {
      ...
      if (pendingCode && pendingState === 'gmail_connect') {
        ...
        navigate(`/admin?code=${pendingCode}&state=${pendingState}`, {
          replace: true
        });
      } else {
        // Normal redirect
  ...
  if (authLoading) {
    return <div className="container max-w-md mx-auto py-8">
        <Card>
          <CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setVerificationEmailSent(false)}>
  ...
  return <div className="container max-w-md mx-auto py-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {mode === "signin" ? "Logg inn for å få tilgang til alle funksjoner." : "Opprett en konto for å få tilgang til alle funksjoner."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {error && <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 rounded-md flex items-start">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={isSubmitting} className="w-full text-slate-950">
              {isSubmitting ? "Vennligst vent..." : mode === "signin" ? "Logg inn" : "Registrer"}
            </Button>
          </form>
          <div className="mt-4 text-center text-sm">
            {mode === "signin" ? <p>
                Har du ikke en konto?{" "}
                <Button variant="link" className="p-0" onClick={() => navigate(`/auth?mode=signup&redirect=${encodeURIComponent(redirectPath)}`)}>
                  Registrer deg her
                </Button>
              </p> : <p>
                Har du allerede en konto?{" "}
                <Button variant="link" className="p-0" onClick={() => navigate(`/auth?mode=signin&redirect=${encodeURIComponent(redirectPath)}`)}>
                  Logg inn her
                </Button>
              </p>}
          </div>
